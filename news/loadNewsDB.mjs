...
const loadNews = async () => {
  const stopwords = [ 'stop', 'the', 'to', 'and', 'a', 'in', 'it', 'is', 'I', 'that', 'had', 'on', 'for', 'were', 'was']
  ...
  fetch("https://penn-cis545-files.s3.amazonaws.com/News_Category_Dataset_v2.json")
   ...
   .then((response) => {
      ...
      entries.forEach(e => {
         ...
         const keywords = e.short_description.split(' ').filter(
            word => !stopwords.includes(word)
         ).filter(
            word => word
         ).map(stemmer).map(word => word.toLowerCase()
